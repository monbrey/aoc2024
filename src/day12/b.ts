export const run = (input: string) => {
	...
	const calcPlotSides = (plot: [number, number][]) => {
		// With the coordinates in place, we're going to try to trace out the perimeter by going left/forward/right/reverse at every opportunity
		const sides = [];
	...
	return plots.reduce((acc, plot) => acc += plot.length * calcPlotSides(plot), 0);
}